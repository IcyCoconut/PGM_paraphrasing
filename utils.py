...
class BufferHandler(object):
    """Buffer handler for output
        Usage:
            Handle writing of output paraphrase of the program on disk.
    """

    def __init__(self, file_name, buffer_size=-1, flush_interval=1):
        '''
        ...
        '''
        self.fileobject = open(file_name, mode='a+', buffering=buffer_size)
    ...
    def appendtext(self, text):
        ...
        if (time.time() - self.timestamp) // 60 > self.flush_interval:
            # If flush interval is met, flsuh to disk
            print("Flushed to output.....")
